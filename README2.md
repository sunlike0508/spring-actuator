...
그리고 스프링 부트 액츄에이터는 마이크 로미터가 제공하는 지표 수집을 `@AutoConfiguration` 을 통해 자동으로 등록해준다.
...
그런데 이러한 메트릭들을 어딘가에 지속 해서 보관해야 과거의 데이터들도 확인할 수 있을 것이다. 
...
프로메테우스와 그라파나는 그 내용만 다루는 책이 있을 정도로 내용이 방대하다. 

프로메테우스와 그라파나에 대한 자세한 내용은 강의 범위를 벗어난다. 

여기서는 각각의 기술들을 어떻게 다루고 활용해야 하는지, 기초 내용과 올바른 방향을 설명하는데 초점을 맞춘다. 
